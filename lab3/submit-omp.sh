...
for threads in $THREADS_LIST; do
    ...
    for size in 4096; do
        echo "Running with size $size using $threads threads" | tee -a $OUTPUT_FILE
        JOB_ID=$(sbatch --ntasks=1 --nodes=1 --cpus-per-task=$threads -W --wrap="srun ./$PROG $size")
        wait
