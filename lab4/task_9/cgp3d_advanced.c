...
#else
void ssor_forward_sweep(int local_nx, int local_ny, int local_nz,
                        double* restrict Ax, double w)
{
    #define AX(i,j,k) (Ax[(i) + local_nx * ((j) + local_ny * (k))])
...
void ssor_backward_sweep(int local_nx, int local_ny, int local_nz,
                         double* restrict Ax, double w)
{
    #define AX(i,j,k) (Ax[(i) + local_nx * ((j) + local_ny * (k))])
    for (int k = local_nz - 1; k >= 0; --k) {
        for (int j = local_ny - 1; j >= 0; --j) {
            for (int i = local_nx - 1; i >= 0; --i) {
                double xx = AX(i,j,k);
                double xs = (i < local_nx - 1)   ? AX(i+1,j,k) : 0;
                double xw = (j < local_ny - 1)   ? AX(i,j+1,k) : 0;
                double xd = (k < local_nz - 1)   ? AX(i,j,k+1) : 0;
                AX(i,j,k) = (xx + xs + xw + xd) * w / 6.0;
            }
        }
    }
    #undef AX
...
void ssor_diag_sweep(int local_nx, int local_ny, int local_nz,
                     double* restrict Ax, double w)
{
    #define AX(i,j,k) (Ax[(i) + local_nx * ((j) + local_ny * (k))])
    double factor = 6.0 * (2.0 - w) / w;
    for (int k = 0; k < local_nz; ++k) {
        for (int j = 0; j < local_ny; ++j) {
            for (int i = 0; i < local_nx; ++i) {
                AX(i,j,k) *= factor;
 ...
 *@c*/
void schwarz_get(int n, int i_start_global, int j_start_global, int k_start_global,
                 ...
                 double* restrict x)
{
    int idx = 0;
    for (int k = 0; k < local_nz; ++k) {
        int global_k = k + k_start_global;
        for (int j = 0; j < local_ny; ++j) {
            int global_j = j + j_start_global;
            for (int i = 0; i < local_nx; ++i) {
                int global_i = i + i_start_global;
                int global_idx = global_i + n * (global_j + n * global_k);
                x_local[idx] = x[global_idx];
                idx++;
            }
        }
    }
 ...
 *@c*/
void schwarz_add_atomic(int n, int local_nx, int local_ny, int local_nz,
                        int i_start_global, int j_start_global, int k_start_global,
                        double* local_scratch,
                        double* Ax)
{
    int idx = 0; // Local index
    for (int k = 0; k < local_nz; ++k) {
        int global_k = k + k_start_global;
        for (int j = 0; j < local_ny; ++j) {
            int global_j = j + j_start_global;
            for (int i = 0; i < local_nx; ++i) {
                ...
                int global_idx = global_i + n * (global_j + n * global_k);
                #pragma omp atomic
                Ax[global_idx] += local_scratch[idx];
                idx++;
            }
        }
    }
                          ...
                          double* restrict x)
{
    ...
    int n = ssor_data->n;
    int overlap = ssor_data->overlap / 2;
    double w = ssor_data->omega;
    int subdomain_size = (n + P - 1) / P; // Ceiling division

    // Initialize Ax to zero
    memset(Ax, 0, N * sizeof(double));

    // Allocate arrays
    double** scratch = (double**) malloc(P * sizeof(double*));
    int* z_start = (int*) malloc(P * sizeof(int));
    int* z_end = (int*) malloc(P * sizeof(int));

    // Precompute subdomain boundaries
    for (int p = 0; p < P; ++p) {
        z_start[p] = p * subdomain_size - overlap;
        z_end[p] = (p + 1) * subdomain_size + overlap;

        if (z_start[p] < 0) z_start[p] = 0;
        if (z_end[p] > n) z_end[p] = n;

        int local_nz = z_end[p] - z_start[p];
        int local_nx = n;
        int local_ny = n;
        int local_size = local_nx * local_ny * local_nz;
        scratch[p] = (double*) malloc(local_size * sizeof(double));
    }

    // Parallel computation over subdomains
    #pragma omp parallel for
    for (int p = 0; p < P; ++p) {
        double* local_scratch = scratch[p];

        int local_nx = n;
        int local_ny = n;
        int local_nz = z_end[p] - z_start[p];

        int i_start_global = 0;
        int j_start_global = 0;
        int k_start_global = z_start[p];

        // Extract local data
        schwarz_get(n, i_start_global, j_start_global, k_start_global,
                    local_nx, local_ny, local_nz,
                    local_scratch, x);

        // Perform SSOR sweeps
        ssor_forward_sweep(local_nx, local_ny, local_nz,
                           local_scratch, w);
        ssor_diag_sweep(local_nx, local_ny, local_nz,
                        local_scratch, w);
        ssor_backward_sweep(local_nx, local_ny, local_nz,
                            local_scratch, w);
    }
    ...
    for (int p = 0; p < P; ++p) {
        ...
        schwarz_add_atomic(n, local_nx, local_ny, local_nz,
