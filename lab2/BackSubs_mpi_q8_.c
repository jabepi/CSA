/* C Example of Backward Substitution */
#define _XOPEN_SOURCE
#include <stdio.h>
#include <stdlib.h>
#include <omp.h>
#include <mpi.h>
#include <time.h>
#include <math.h>
   
/* header files for getting hostname and process id */ 
#include <unistd.h>
#include <sys/types.h>
   
/* Get time and resources */     
#include <sys/time.h>   
#include <sys/resource.h> 
   
//#define _DEBUG_  
//#include "constants.h"   
   
#if _EXTRAE_   
#include "extrae_user_events.h"    
// Extrae Constants    
#define  PROGRAM    1000   
#define  END    0  
#define  SERIAL 1  
#define  PARALLEL   2  
#else  
double getusec_() {    
    struct timeval time;   
    gettimeofday(&time, NULL);     
    return ((double)time.tv_sec * (double)1e6 + (double)time.tv_usec); 
}    
   
#define START_COUNT_TIME stamp = getusec_();   
#define STOP_COUNT_TIME(_m) stamp = getusec_() - stamp;\
    stamp = stamp/1e6;\
    printf ("Time: %s%0.6f\n",(_m), stamp);  
#endif

#ifdef SINGLE_PRECISION

typedef float fp_t;

#else

typedef double fp_t;

#endif 
  
int m; /* order of symmetric input matrix */
int n; /* number of right-hand sides */
int b; /* block size */
int reps; /* number of repetitions */
int check; /* check result */
fp_t alpha;
int lda;
int ldb;
char uplo; /* lower/upper triangular */
char side;
char trans;
char diag;

fp_t *A_local, *A;
fp_t *B_local, *B;
fp_t *Bchk;   
   
void GENMAT_IP(fp_t *A, int m, int n, int scale, int seed) 
{
	srand48(time(NULL)+seed);

	int j;
	for (j = 0; j < n; ++j ) {
		int i;
		for( i = 0; i < m; ++i ) {
			A[j*m+i] = (fp_t) drand48();
			if ( j == i )
				A[j*m+i] += scale;
		}
  	}
}

int trsm_setup(int check, int m, int n, int b, int lda, int ldb, fp_t **A, fp_t **B, fp_t **Bchk) 
{
	fp_t *lA = *A = malloc(lda*m*sizeof(fp_t));
	if ( lA == NULL )
		return 1;

	GENMAT_IP(lA, lda, m, 1,0);
    int j, i;
	for (j = 0; j < m; ++j ) 
		for( i = 0; i < j; ++i ) 
			lA[j*m+i] = (fp_t) 0.0;

	fp_t *lB = *B = malloc(ldb * n * sizeof(fp_t));
	if (lB == NULL)
		return 2;
	GENMAT_IP(lB, ldb, n, 1,1);

	fp_t *lBchk = *Bchk = malloc(ldb * n * sizeof(fp_t));
	if (lBchk == NULL)
		return 3;

	for ( i = 0; i < ldb*n; ++i )
		lBchk[i] = lB[i];

	return 0;
}

void trsm_shutdown(fp_t *A, fp_t *B, fp_t *X) 
{
	free(A);
	free(B);
	free(X);
}    
    

int main(int argc, char *argv[]) {
    int  myid, numprocs; 
#ifdef _DEBUG_ 
    char hostname[128];
#endif
    MPI_Status status;

    const char Usage[] = "Usage: BackSubs <size> (try 10000)\n";
    if (argc < 2) {
    fprintf(stderr, Usage);
    exit(1);
    }
    unsigned long long int size = atoll(argv[1]);
    
    side = 'L';
    uplo = 'U';

    m = size;
    n = size;
    b = 1;
    lda = size;
    ldb = 1;

    alpha = 1.0;
    trans = 'N';
    diag = 'N';
    reps = 1;
    check = 1;
    
    // Initialize the MPI environment
    MPI_Init(&argc, &argv); 
    MPI_Comm_size(MPI_COMM_WORLD, &numprocs);  // Get the total number of processes
    MPI_Comm_rank(MPI_COMM_WORLD, &myid);      // Get the rank of the current process


    // Allocate memory only for the rows assigned to this process
    int num_mylines = 0;
    for (int i = myid; i < size; i += numprocs) {
        num_mylines++;
    }

    A_local = malloc(lda * num_mylines * sizeof(fp_t));  // Only allocate space for this process's rows of A
    B_local = malloc(num_mylines * sizeof(fp_t));

    if (A_local == NULL || B_local == NULL) 
    return 1;

    int local_row_idx = 0;  // Local index for storing rows of A and elements of B

// Process 0 initializes the matrices A and B
if (myid == 0) { 
   if (trsm_setup(check, m, n, b, lda, ldb, &A, &B, &Bchk)) {
        fprintf(stderr, "err: allocating matrix\n");
        return 2;
   }

   // Process 0 sends the relevant rows of A and B to each process
   for (int i = 0; i < size; i++) {
       int owner = i % numprocs;  // Determine which process owns row i
       if (owner == 0) {
           // Keep rows assigned to process 0
           memcpy(&A_local[local_row_idx * lda], &A[i * lda], lda * sizeof(fp_t)); 
           B_local[local_row_idx] = B[i];
           local_row_idx++;
       } else {
           // Send the row A[i] and B[i] to the appropriate process
           MPI_Send(&A[i * lda], lda, MPI_DOUBLE, owner, 0, MPI_COMM_WORLD);  // Send row i of A
           MPI_Send(&B[i], 1, MPI_DOUBLE, owner, 0, MPI_COMM_WORLD);           // Send corresponding B[i]
       }
   }
    } else {
        // Other processes receive their assigned rows of A and elements of B
        for (int i = myid; i < size; i += numprocs) {
            MPI_Recv(&A_local[local_row_idx * lda], lda, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD, &status);  // Receive row i of A
            MPI_Recv(&B_local[local_row_idx], 1, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD, &status);          // Receive B[i]
            local_row_idx++;
        }
    }

#if _EXTRAE_
    Extrae_event (PROGRAM, SERIAL);
#endif

#if _EXTRAE_
    Extrae_event (PROGRAM, END);
#endif

    /* do computation -- using all available threads */
#if _EXTRAE_
    Extrae_event (PROGRAM, PARALLEL);
#endif

    // Broadcast the full matrix A and vector B from process 0 to all processes
    MPI_Bcast(A, lda * m, MPI_DOUBLE, 0, MPI_COMM_WORLD); 
    MPI_Bcast(B, ldb * n, MPI_DOUBLE, 0, MPI_COMM_WORLD);

#if _EXTRAE_
#else
    double stamp;
    if (myid == 0) START_COUNT_TIME;
#endif

    // Backward substitution loop with cyclic partitioning
    int i, j;
    for (i = size - 1; i >= 0; i--) {
        int owner = i % numprocs;  // Determine which process owns row i

        // The owner process solves for B[i]
        if (myid == owner) {
            // Find the local index of the row for this process
            int local_i = (i - myid) / numprocs;
            B_local[local_i] = B_local[local_i] / A_local[local_i * lda + i];  // Solve for B[i]
            
            // Broadcast the computed value of B[i] to all other processes
            MPI_Bcast(&B_local[local_i], 1, MPI_DOUBLE, owner, MPI_COMM_WORLD);
        } else {
            // All other processes receive B[i] from the owner
            MPI_Bcast(&B[i], 1, MPI_DOUBLE, owner, MPI_COMM_WORLD);
        }

        // Update the rows below i that belong to this process
        for (j = myid; j < i; j += numprocs) {  // Cyclic iteration, process works on its assigned rows
            int local_j = (j - myid) / numprocs;  // Local index for row j
            B_local[local_j] = B_local[local_j] - B[i] * A_local[local_j * lda + i];  // Subtract contribution of B[i]
        }
    }

 
#if _EXTRAE_
    Extrae_event (PROGRAM, END);
    Extrae_event (PROGRAM, SERIAL);
#else
    if (myid == 0) { STOP_COUNT_TIME("");}
#endif

#if _EXTRAE_
    Extrae_event (PROGRAM, END);
#endif

    if (myid == 0) {
        if (size<=1000) {
           if (size<=10) { // Simple printing code for debug purposes    
              for ( i = 0; i < lda; ++i ) { 
                for ( j = 0; j < m; ++j ) 
                  fprintf(stdout,"%lf ",A[i*m+j]);
              fprintf(stdout,"\n");
	      }
              fprintf(stdout,"\n");
              for ( i = 0; i < ldb*n; ++i ) 
                  fprintf(stdout,"%lf ",B[i]);
              fprintf(stdout,"\n");
 	      for ( i = 0; i < ldb*n; ++i ) 
                  fprintf(stdout,"%lf ",Bchk[i]);
              fprintf(stdout,"\n");
           }
        fp_t error=0;
        fp_t suma;
        for (i=size-1; i>=0; i--) {
            suma=0.0;
            for (j=i; j<=size-1; j++)
                suma+=B[j]*A[i*m+j];
            if (error<fabs(Bchk[i]-suma)) {
                error=fabs(Bchk[i]-suma);
		fprintf(stdout,"%d:%lf vs %lf -> %lf\n",i,suma,Bchk[i],error);
	    }
        }
        fprintf(stdout,"Error: %lf\n",error);
        }
    }

    if (myid == 0) {
        trsm_shutdown(A, B, Bchk);
    }else {
        free(A);
        free(B);
    }
    
#ifdef _DEBUG_ 
    gethostname(hostname, 126);
    printf( "Hello world from process %d of %d at hostname %s Processes lines %d to %d\n",myid, numprocs, hostname, mylinesinit, mylinesend );
#endif

    MPI_Finalize(); 

    return EXIT_SUCCESS;
}
